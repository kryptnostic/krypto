...
public:
  ...
  const BitVector<N> getMetadataAddressFromPair(const BitVector<N> &token, const std::pair<BitVector<2*N>, BitMatrix<N>> & objectSearchPair, const PrivateKey<N,MON> & pk) const {
   ...
   */
  template<unsigned int MON>
  const ClientHashFunction<N> getClientHashFunction(const PrivateKey<N,MON> & pk) const {
    ClientHashFunction<N> h;
    h.initialize(_K * BitMatrix<N,2*N>::augH(BitMatrix<N>::identityMatrix(), _R), pk);
    return h;
  }
   ...
   */
  template<unsigned int MON>
  const std::pair<BitVector<2*N>, BitMatrix<N>> getObjectSearchPairFromObjectIndexPair(const std::pair<BitVector<N>, BitMatrix<N>> & objectIndexPair, const PrivateKey<N,MON> & pk) const {
   ...
   */
  template<unsigned int MON>
  const std::pair<BitVector<N>, BitMatrix<N>> getObjectIndexPairFromObjectSearchPair(const std::pair<BitVector<2*N>, BitMatrix<N>> & objectSearchPair, const PrivateKey<N,MON> & pk) const {
   ...
   */
  template<unsigned int MON>
  const std::pair<BitVector<N>, BitMatrix<N>> getObjectSharePairFromObjectSearchPair(const std::pair<BitVector<2*N>, BitMatrix<N>> & objectSearchPair, const PrivateKey<N,MON> & pk) const {
   ...
   */
  template<unsigned int MON>
  const std::pair<BitVector<2*N>, BitMatrix<N>> getObjectSearchPairFromObjectSharePair(const std::pair<BitVector<N>, BitMatrix<N>> & objectSharePair, const PrivateKey<N,MON> & pk) const {
