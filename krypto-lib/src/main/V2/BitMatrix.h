...
/* Generation */
	...
	/*
	 * Function: allOneMatrix()
	 * Returns the all one matrix of size ROWS x COLS
	 */
	static const BitMatrix<ROWS,COLS> allOneMatrix() {
		BitMatrix<ROWS,COLS> result;
		for (unsigned int i = 0; i < ROWS; ++i)
		{
			result._rows[i] = BitVector<COLS>::allOneVector();
		}
		return result;
	}
    ...
    /*
     * Function: generateSparseInvertibleMatrix(num)
     * Generate a sparse invertibe matrix, where each row has at least one and at most num number of non-zero entries
     * e.g. to generate a permutation matrix, use generateSparseInvertibleMatrix(1)
     */
    static const BitMatrix<ROWS> generateSparseInvertibleMatrix(const unsigned int num) {
		BitMatrix<ROWS> result = BitMatrix<ROWS>::identityMatrix();
		...
		if (num > 1)
		{
			for (int i = 0; i < ROWS - 1; ++i)
			{
				...
				unsigned int nnz = ran % min(num, ROWS - i);
				for (int j = 0; j < nnz; ++j)
				{
					int b = i;
					while (b <= i || result.get(i, b)) 
					{
						std::fread(&ran, sizeof( unsigned int),1,urandom);
						b = ran % ROWS;
					}
...
/* Operators */
	...
	/*
	 * Function: |
	 * Returns ther result of matrix OR matrix operation
	 */
	const BitMatrix<ROWS,COLS> operator| (const BitMatrix<ROWS,COLS> & rhs) const{
		BitMatrix<ROWS,COLS> result = *this;
		for (size_t i = 0; i < ROWS; ++i) {
			result._rows[i] = result._rows[i] | rhs._rows[i];
		}
		return result;
	}
...
/* Elementary row operations */
	...
	/*
	 * Function: Reverse()
	 * Obtains the matrix by doing a NOT operation, i.e. 0 -> 1 and 1 -> 0
	 * Used in MonomialMatrix.h
	 */
	void reverse() {
		for (int j = 0; j < COLS; ++j)
		{
	...
	/*
	 * Function: randomSwap()
	 * randomly swaps the rows and columns of matrix
	 */
	void randomSwap() {
		unsigned int ran;
		std::fread(&ran, sizeof( unsigned int),1,urandom);
		int num = ran % (ROWS * ROWS);
		for (int i = 0; i < num; ++i)
		{
			std::fread(&ran, sizeof( unsigned int),1,urandom);
			int rowIndex = ran % ROWS;
...
/* Matrix-vector Operations */
	...
	const BitVector<ROWS> solve (const BitVector<ROWS> & rhs, bool & solvable) const{
		...
		for (int k = 0; k < ROWS; ++k) {
			...
			if (pos != -1) {
				...
				throw std::invalid_argument("Error: solving system of a singular matrix!");
			}
		}
	...
	/*
	 * Function: andEachRow(input)
	 * Returns the BitMatrix obtained by AND-ing each row of the matrix with input
	 * i.e. A AND [input | ... | input]^T
	 */
	const BitMatrix<ROWS,COLS> andEachRow(const BitVector<COLS> & input) const {
		BitMatrix<ROWS,COLS> result = BitMatrix<ROWS,COLS>::zeroMatrix();
		for (int i = 0; i < ROWS; ++i)
		{
			result.setRow(i, _rows[i] & input);
		}
		return result;
	}
	...
	/*
	 * Function: eval(x)
	 * Returns the BitVector obtained by evaluating the polynomial represented by the matrix at x (see documentation)
	 */
	template<unsigned int NUM_INPUT>
	const BitVector<COLS> eval(const BitVector<NUM_INPUT> & x) const {
		if (DEBUG) assert((((NUM_INPUT * (NUM_INPUT + 1)) >> 1) + 1) == ROWS);
		BitVector<COLS> result = _rows[ROWS-1];
		int currModifier = 0;
		...
		for (int i = 0; i < NUM_INPUT; ++i)
		{
			if (x[i])
			{
				for (int j = 0; j < NUM_INPUT - i; ++j)
				{
					if (x[i + j])
					{
						result ^= _rows[currModifier + j];
					}
				}
			}
			currModifier += NUM_INPUT - i;
		}
	...
	/*
	 * Function: eval(x, y)
	 * Returns the BitVector obtained by evaluating the (bivariate) polynomial represented by the matrix at x and y (see documentation)
	 */
	template<unsigned int NUM_INPUT1, unsigned int NUM_INPUT2>
	const BitVector<COLS> eval(const BitVector<NUM_INPUT1> & x, const BitVector<NUM_INPUT2> & y) const {
		if (DEBUG) assert(NUM_INPUT1 * NUM_INPUT2 == ROWS);
		BitVector<COLS> result = BitVector<COLS>::zeroVector();
		int currModifier = 0;

		for (int i = 0; i < NUM_INPUT1; ++i)
		{
			if (x[i])
			{
				for (int j = 0; j < NUM_INPUT2; ++j)
				{
					if (y[j])
					{
						result ^= _rows[currModifier + j];
...
/* Multi-Matrix Functions */
	...
	template <unsigned int ROWST>
	static const BitMatrix<ROWST+1, COLS> augV(const BitMatrix<ROWST, COLS> & top, const BitVector<COLS> & bot) { //TODO: optimize
        BitMatrix<ROWST+1, COLS> result;
...
/* Print */
	...
	void printRow(unsigned int rowIndex) const {
		...
		cout << /*"Row " << rowIndex << ":\t" <<*/ "|" << get(rowIndex, 0);
	...
	// Returns the maximum number of nonzero entry in a row
	const unsigned int maxnnz() const {
		unsigned int res = 0;
		for (int i = 0; i < ROWS; ++i)
		{
			unsigned int curr = _rows[i].nnz();
			if (res < curr) res = curr;
		}
		return res;
	}

	// FOR SELF, REMOVE WHEN COMMITTING
	// const unsigned int totalnnz() const {
	// 	unsigned int res = 0;
	// 	for (int i = 0; i < ROWS; ++i)
	// 	{
	// 		res += _rows[i].nnz();
	...
	// const double avgnnz() const {
	// 	double res = 0;
	// 	for (int i = 0; i < ROWS; ++i)
	// 	{
	// 		if (!_rows[i].isZero()) res += _rows[i].nnz() / (double) COLS;
	// 	}
	// 	return res / (double) (ROWS - numZeroRow());
	// }

	// const unsigned int numZeroRow() const {
	// 	unsigned int res = 0;
	// 	for (int i = 0; i < ROWS; ++i)
	// 	{
	// 		if (_rows[i].isZero()) res++;
	// 	}
	// 	return res;
	// }

	// static const BitMatrix<ROWS,COLS> makeMatrix(const BitVector<ROWS> & x, const BitVector<COLS> & y) {
	// 	BitMatrix<ROWS,COLS> result = BitMatrix<ROWS,COLS>::zeroMatrix();
	// 	for (int i = 0; i < ROWS; ++i)
	// 	{
	// 		if (x.get(i)) result.setRow(i, y);
	// 	}
	// 	return result;
	// }
