...
TEST(KryptnosticServerTest, testInit){
	...
	std::pair<BitVector<2*N>, BitMatrix<N> > objectSearchPair = sk.getObjectSearchPair(objectSearchKey, objectAddressFunction, pk);

	BitVector<> token = BitVector<>::randomVector();
	BitVector<> expectedAddress = sk.getMetadataAddress(objectAddressFunction, objectSearchKey, token);
